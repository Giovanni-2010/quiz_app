...
try:
    ...
    CTkMessagebox(title="Error", message="Failed to load questions.", icon="cancel")
    sys.exit(1)
...
def save_settings():
    settings = {
        ...
        "time_per_question": int(time_per_question.get())
    }
...
def countdown(seconds):
    global timer_id
    if seconds >= 0:
        timer.configure(text=f"{seconds:02}")
...
def add_score_info(text):
    score_box.configure(state="normal")
    score_box.delete("1.0", "end")
    score_box.insert("1.0", text)
    score_box._textbox.tag_configure("right", justify="right")
    score_box._textbox.tag_add("right", "1.0", "end")
...
# ==================== Plotting ====================
def plot_bar_chart(parent, x, y, xlabel, ylabel, title,margin):
    ...
    for i, val in enumerate(y):
        ax.text(i, val / 2 if val > 0 else 0.1, f'{int(val)}', ha='center', va='center', color='white', fontsize=8, fontweight='bold')

    canvas = FigureCanvasTkAgg(fig, master=parent)
    canvas.draw()

    widget = canvas.get_tk_widget()
    widget.configure(width=700, height=450) 
...
general_tab = score_tabs.add("General")
...
scores_range = highest - lowest
std = np.std(scores)
p25 = np.percentile(scores, 25)
p75 = np.percentile(scores, 75)

score_box = ctk.CTkTextbox(general_tab, width=700, height=350, font=main_font, wrap="word", fg_color="light blue", text_color="black",corner_radius=10, state="disabled")
score_box.pack(pady=10, padx=20, anchor="center")
text = f"لقد لعبت {total_games} لعبة في المجمل.\n نتيجتك الأخيرة كانت {last:.0f} نقطة.\n أقل نتيجة حصلت عليها كانت {lowest:.0f} — كلنا نمر بأيام صعبة!\n أعلى نتيجة حصلت عليها كانت {highest:.0f} — إنجاز رائع!\n معظم نتائجك كانت بين {p25:.0f} و {p75:.0f} نقطة.\n متوسط نتائجك كان حوالي {average:.1f} نقطة.\n نصف نتائجك كانت أعلى من {median:.1f}، والنصف الآخر أقل.\n  تبقى نتائجك ضمن حوالي {std:.1f} نقطة من المتوسط — هذا يعني أنك ثابت بشكل جيد!\n بشكل عام، تراوحت نتائجك بين {lowest:.0f} و {highest:.0f}، أي بفارق قدره {scores_range:.0f} نقطة."
add_score_info(arbic(text))
...
if not df.empty:
    ...
    df_month = df_month[df_month['month_num'] > 0].sort_values('month_num').reset_index(drop=True)
    ...
    df_day = df_day[df_day['day_num'] > 0].sort_values('day_num').reset_index(drop=True)
else:
    df_month = pd.DataFrame(columns=['month', 'score'])
    df_year = pd.DataFrame(columns=['year', 'score'])
    df_day = pd.DataFrame(columns=['day', 'score'])

plot_bar_chart(month_tab, df_month['month'], df_month['score'], "Month", "Score", "Total Score by Month",0.15)
plot_bar_chart(year_tab, df_year['year'], df_year['score'], "Year", "Score", "Total Score by Year",0.15)
plot_bar_chart(day_tab, df_day['day'], df_day['score'], "Day of Week", "Score", "Total Score by Day",0.25)
